...
class TestEvaluationRoutes:
    async def test_routes_exist(self, app: FastAPI, client: AsyncClient) -> None:
        ...
        res = await client.post(
            app.url_path_for("evaluations:create-evaluation-for-cleaner", cleaning_id=uuid_fake, username="bradpitt")
        )
        assert res.status_code != status.HTTP_404_NOT_FOUND
        res = await client.get(
            app.url_path_for("evaluations:get-evaluation-for-cleaner", cleaning_id=uuid_fake, username="bradpitt")
        )
        assert res.status_code != status.HTTP_404_NOT_FOUND
...
class TestCreateEvaluations:
    # o proprietário pode deixar a avaliação para o limpador e marcar a oferta como concluída
...
Though the cleaner took their time, I would definitely hire them again for the quality of their work.
    ...
    # não proprietário não pode deixar comentário
    ...
    # o dono não pode deixar comentário para o usuário errado
    ...
    # o proprietário não pode deixar vários comentários
...
class TestGetEvaluations:
    ...
    """
    # usuário autenticado pode obter avaliação para limpeza
    ...
    # o usuário autenticado pode obter uma lista de avaliações para limpeza
    ...
    # o usuário autenticado pode obter estatísticas agregadas para limpeza
    ...
    # usuário não autenticado proibido de obter solicitações
