...
class TestCreateCleaning:
    ...
    async def test_invalid_input_raises_error(
        ...
        res = await client.post(
            ...
            app.url_path_for("cleanings:create-cleaning"), json=invalid_payload
        )
...
class TestGetCleaning:
    ...
    @pytest.mark.parametrize(
        "id, status_code",
        (
            (str(uuid4()), 404),        # id (uuid) not exists in db
            (-1, 422),                  # id (uuid) cat't negative number
            (101, 422),                 # id (uuid) can't number
            (None, 422),                # id (uuid) can't none
        ),
    )
    async def test_wrong_id_returns_error(
        self, app: FastAPI, client: AsyncClient, id: UUID, status_code: int
    ) -> None:
...
class TestUpdateCleaning:
    ...
    @pytest.mark.parametrize(
        "payload, status_code",
        (
            ({"cleaning_type": None}, 400),
        )
    )
    async def test_update_cleaning_with_input_violates_not_null_throws_error(
        self,
        app: FastAPI,
        client: AsyncClient,
        payload: dict,
...
class TestDeleteCleaning:
    ...
    async def test_cant_delete_cleaning_with_invalid_ids_throws_error(
        self, app: FastAPI, client: AsyncClient, id: UUID, status_code: int
    ) -> None:
