...
class CleaningsRepository(BaseRepository):
    ...
    async def get_cleaning_by_id(self, *, id: UUID) -> CleaningInDB:
    ...
    async def get_all_cleanings(self) -> List[CleaningInDB]:
        cleanings_records = await self.db.fetch_all(query=GET_ALL_CLEANINGS_QUERY)

        return [CleaningInDB(**r) for r in cleanings_records]

    async def update_cleaning(self, *, id: UUID, cleaning_update: CleaningUpdate) -> CleaningInDB:
    ...
    async def delete_cleaning_by_id(self, *, id: UUID) -> UUID:
        cleaning = await self.get_cleaning_by_id(id=id)
